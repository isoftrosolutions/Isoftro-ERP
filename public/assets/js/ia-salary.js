...
async function loadStaffListForForm() {
    ...
    try {
        const [resT, resF] = await Promise.all([
            fetch(window.APP_URL + '/api/admin/staff?role=teacher'),
            fetch(window.APP_URL + '/api/admin/staff?role=frontdesk')
        ]);
...
async function loadSalaryForEdit(id) {
    try {
        const res = await fetch(window.APP_URL + '/api/admin/salary?id=' + id);
