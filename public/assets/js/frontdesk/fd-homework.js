    ...
    try {
        ...
        if (data.success && data.data) {
            ...
            if(sel) {
                ...
                data.data.forEach(c => html += `<option value="${c.id}">${c.name}</option>`);
                sel.innerHTML = html;
            }
        }
...
window.iaHwLoadBatches = async function() {
    ...
    try {
        ...
        if (data.success && data.data) {
            ...
            data.data.forEach(b => html += `<option value="${b.id}">${b.name}</option>`);
            sel.innerHTML = html;
        }
...
window.loadHomeworkData = async function() {
    ...
    tbody.innerHTML = '<tr><td colspan="6" style="text-align:center; padding:20px"><i class="fa fa-spinner fa-spin"></i> Loading...</td></tr>';
    
    try {
        /*
         * Note: API endpoint for homework will be created next
         */
        const url = APP_URL + `/api/frontdesk/homework?course_id=${courseId}&batch_id=${batchId}&status=${status}`;
        const res = await fetch(url);
        
        // Handle mock response or actual response properly
        if(res.status === 404) {
             tbody.innerHTML = '<tr><td colspan="6" style="text-align:center; padding:20px; color:var(--text-light)">Endpoint /api/frontdesk/homework not created yet.</td></tr>';
             return;
        }
        ...
        if (data.success) {
            if (data.homework && data.homework.length > 0) {
                ...
                tbody.innerHTML = '<tr><td colspan="6" style="text-align:center; padding:20px; color:var(--text-light)">No homework found matching criteria.</td></tr>';
            }
        } else {
...
window.iaHwFormLoadBatches = async function() {
    ...
    document.getElementById('formSubject').innerHTML = '<option value="">Select Subject</option>';
...
window.iaHwFormLoadSubjects = async function() {
    ...
    try {
        ...
        if(res.status === 404) {
             ...
             let html = '<option value="">Select Subject</option><option value="1">Mathematics</option><option value="2">Science</option><option value="3">English</option>';
             sel.innerHTML = html;
             return;
        }
        ...
        if (data.success && data.data) {
            ...
            data.data.forEach(s => html += `<option value="${s.id}">${s.name} (${s.code || '-'})</option>`);
            sel.innerHTML = html;
        }
    } catch(e) { 
