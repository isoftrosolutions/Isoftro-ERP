...
window.loadStaff = async function(role) {
    ...
    try {
        const res = await fetch(`${APP_URL}/api/frontdesk/staff?role=${role}`, { credentials:'include', headers:{'X-CSRF-TOKEN':document.querySelector('meta[name="csrf-token"]')?.content||''} });
...
function _renderEditStaffForm(role, staff) {
    ...
    document.getElementById('editStaffForm').onsubmit = async function(e) {
        ...
        try {
            const res = await fetch(APP_URL+'/api/frontdesk/staff',{method:'PUT',credentials:'include',headers:{'Content-Type':'application/json','X-CSRF-TOKEN':document.querySelector('meta[name="csrf-token"]')?.content||''},body:JSON.stringify(data)});
...
window.renderAddStaffForm = function(role) {
    ...
    document.getElementById('addStaffForm').onsubmit = async function(e) {
        ...
        try {
            const res = await fetch(APP_URL+'/api/frontdesk/staff',{method:'POST',credentials:'include',headers:{'Content-Type':'application/json','X-CSRF-TOKEN':document.querySelector('meta[name="csrf-token"]')?.content||''},body:JSON.stringify(data)});
            const result = await res.json();
            if (result.success) {
...
window.deleteStaff = async function(role, userId, name) {
    ...
    try {
        const res = await fetch(APP_URL+'/api/frontdesk/staff',{method:'DELETE',credentials:'include',headers:{'Content-Type':'application/json','X-CSRF-TOKEN':document.querySelector('meta[name="csrf-token"]')?.content||''},body:JSON.stringify({user_id:userId,role})});
