...
window.loadStudentStats = async () => {
    try {
        const res = await fetch(window.APP_URL + '/api/admin/students?stats=1');
...
window.exportStudentsCSV = async () => {
    ...
    window.location.href = window.APP_URL + '/api/admin/students?' + params.toString();
};
...
window.openBulkEmailModal = () => {
    ...
    document.getElementById('bulkEmailForm').addEventListener('submit', async (e) => {
        ...
        try {
            const res = await fetch(window.APP_URL + '/api/admin/students', {
                method: 'POST',
...
window.renderStudentList = async () => {
    ...
    if (!document.getElementById('ia-students-premium-css')) {
        ...
        link.rel = 'stylesheet';
        link.href = window.APP_URL + '/public/assets/css/ia-students-premium.css';
...
window.loadAlumni = async (page) => {
    ...
    try {
        ...
        const res = await fetch(APP_URL + '/api/admin/students?' + params);
        const result = await res.json();
...
window.renderAddStudentForm = async () => {
    ...
    if (adInput && bsInput) {
        const handleConv = async (e, type) => {
            ...
            const res = await fetch(`${window.APP_URL}/api/admin/date-convert?date=${date}&type=${type}`);
            const result = await res.json();
            if (result.success) {
...
async function _populateCoursesInForm(selectId) {
    ...
    if (!sel) return;
    try {
        const res = await fetch(window.APP_URL + '/api/admin/courses');
...
window._loadBatchesForForm = async (courseId, targetSelectId = 'std_batch_select') => {
    ...
    try {
        const res = await fetch(`${window.APP_URL}/api/admin/batches?course_id=${courseId}`);
        const data = await res.json();
...
async function _loadStudentDataForEdit(id) {
    try {
        const res = await fetch(`${window.APP_URL}/api/admin/students?id=${id}`);
        const result = await res.json();
        if (result.success) {
            const s = result.data;
...
window.loadStudents = async (page) => {
    ...
    try {
        ...
        const res = await fetch(window.APP_URL + '/api/admin/students?' + params.toString());
        const result = await res.json();
...
window.bulkDrop = () => {
    ...
    if(confirm(`Are you sure you want to mark ${selected.length} students as dropped?`)) {
        fetch(window.APP_URL + '/api/admin/students', {
            method: 'POST',
            headers: {'Content-Type':'application/json'},
...
window.renderStudentProfile = async (id, activeTab = 'personal') => {
    ...
    try {
        const res = await fetch(`${window.APP_URL}/api/admin/students?id=${id}&include=details`);
        const result = await res.json();
        if (!result.success) throw new Error(result.message);
                ...
                <div class="sp-profile-header">
                    <div class="sp-profile-header-content">
                        ...
                        <div class="sp-actions">
                            <button class="btn btn-success" style="background:#0F172A; color:#fff;" onclick="goNav('students', 'enroll-existing', { student_id: ${s.id} })">
...
function _spCourseTab(s) {
    ...
    return `
        <div style="display:flex; justify-content:space-between; align-items:center;">
            ...
            <button class="btn btn-sm bt" style="height:32px; font-size:12px; padding:0 12px; background:#0F172A; color:#fff;" onclick="goNav('students', 'enroll-existing', { student_id: ${s.id} })">
...
window.openQuickAddStudent = async () => {
    ...
    document.getElementById('quickAddStudentForm').addEventListener('submit', async function(e) {
        ...
        try {
            const response = await fetch(window.APP_URL + '/api/admin/students', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ full_name: name, phone: phone, contact_number: phone, email: email, password: password, batch_id: batch_id, registration_mode: 'quick', registration_status: 'quick_registered' }) });
...
window.renderDocumentVault = async (page = 1, search = '') => {
    ...
    try {
        const res = await fetch(`${window.APP_URL}/api/admin/students?page=${page}&per_page=12&search=${encodeURIComponent(search)}`);
        const result = await res.json();
        if (!result.success) throw new Error(result.message);
