...
window.getCurrencySymbol =
  ...
  function () {
    return (
      window._INSTITUTE_CONFIG?.currency_symbol ||
      window.INSTITUTE_CONFIG?.currency_symbol ||
      "₹"
    );
  };
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  window.getHeaders = (options = {}) => {
    ...
    // Support multiple token sources for compatibility
    const csrfToken =
      window.CSRF_TOKEN ||
      window.csrfToken ||
  ...
  const NAV = window._IA_NAV_CONFIG
    ? window._IA_NAV_CONFIG.map((section) => ({
        sec: section.section,
        items: section.items.map((item) => ({
          ...
          badge: item.badge_key
            ? { val: "?", c: item.badge_key.includes("amber") ? "amber" : "" }
            : null,
          sub: item.sub || null,
        })),
      }))
    : [
        {
          ...
          items: [
            { id: "dashboard", icon: "fa-th-large", l: "Dashboard" },
            {
              ...
              l: "Today's Attendance",
              badge: { val: 12, c: "amber" },
            },
          ],
        },
        {
  ...
  window.goNav = (id, subActive = null, extraParams = "") => {
    ...
    if (extraParams) {
      ...
      if (typeof extraParams === "string") {
        const ep =
          extraParams.startsWith("&") || extraParams.startsWith("?")
            ? extraParams.substring(1)
            : extraParams;
  ...
  function renderSidebar() {
    ...
    NAV.forEach((section) => {
      ...
      section.items.forEach((item) => {
        const isActive =
          activeNav === item.id || activeNav.startsWith(item.id + "-");
        ...
        if (hasSub) {
          ...
          item.sub.forEach((s) => {
            const isSubActive =
              activeNav === s.id || activeNav === `${item.id}-${s.id}`;
    ...
    const items = [
      {
        ...
        icon: "fa-house",
        label: "Home",
        action: "goNav('dashboard')",
      },
      {
        id: "admissions-adm-all",
        icon: "fa-users",
        label: "Students",
        action: "goNav('admissions', 'adm-all')",
      },
      {
        id: "fee-fee-coll",
        icon: "fa-money-bill-wave",
        label: "Fee",
        action: "goNav('fee', 'fee-coll')",
      },
      {
        id: "operations-inq-list",
        icon: "fa-magnifying-glass",
        label: "Inquiries",
        action: "goNav('operations', 'inq-list')",
      },
    ];
  ...
  window.renderPartialModule = async function (endpoint, extraParams = "") {
    ...
    mc.innerHTML =
      '<div class="pg fu"><div class="pg-loading"><i class="fa-solid fa-circle-notch fa-spin"></i><span>Loading Module...</span></div></div>';
    ...
    try {
      ...
      scripts.forEach((s) => {
        try {
          ...
          console.warn(
            `[renderPartialModule] Script error in ${endpoint}:`,
            ex,
          );
        }
      });
  ...
  function renderPage() {
    ...
    mainContent.innerHTML =
      '<div class="pg fu"><div class="pg-loading"><i class="fa-solid fa-circle-notch fa-spin"></i><span>Loading...</span></div></div>';
    ...
    // ── STUDENT MODULES ──
      ...
      activeNav === "students-view" ||
      activeNav === "admissions-view"
    ) {
      ...
      activeNav === "students-edit" ||
      activeNav === "admissions-edit" ||
      activeNav === "students-complete"
    ) {
      ...
      activeNav === "new-admission" ||
      activeNav === "admissions-adm-form"
    ) {
      if (window.renderAddStudentForm) window.renderAddStudentForm();
    } else if (
      activeNav === "enroll-existing" ||
      activeNav === "admissions-enroll-existing"
    ) {
    ...
    // ── FINANCE MODULES ──
      ...
      activeNav === "pending-dues" ||
      activeNav === "finance-fee-outstanding" ||
      ...
      activeNav === "fee-record"
    ) {
      if (window.renderFeeRecord) window.renderFeeRecord();
    } else if (
      activeNav === "fee-details" ||
      activeNav.includes("fee-details")
    ) {
      if (window.renderFeeDetails) {
        // Extract receipt number from URL if possible
        const receiptNo = new URLSearchParams(window.location.search).get(
          "receipt_no",
        );
    ...
    // ── OPERATIONS MODULES ──
      ...
      activeNav === "reception-visitor" ||
      activeNav === "visitor-log"
    ) {
      window.renderPartialModule("visitor-log");
    } else if (
      activeNav === "reception-appointment" ||
  ...
  async function renderDashboard() {
    window.scrollTo(0, 0);
    mainContent.innerHTML =
      '<div class="pg fu"><div class="pg-loading"><i class="fa-solid fa-circle-notch fa-spin"></i><span>Loading Dashboard...</span></div></div>';
                                        ...
                                        <tbody>
                                            ...
                                            <tr class="${i % 2 === 0 ? "" : "even"}">
                                                ...
                                                <td style="color:#6B7A99;font-size:11px;">${t.time || t.created_at ? new Date(t.created_at).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) : "--"}</td>
                                                    ...
                                                    .join("")
                                                : '<tr><td colspan="5" style="text-align:center;padding:30px;color:var(--text-light);">No transactions yet.</td></tr>'
                                            }
                                        </tbody>
                                    </table>
                                </div>
                                ...
                                <div style="margin-top:12px;">
                                    ${(
                                      s.announcements || [
                                        {
                                          title:
                                            "Fee Deadline Reminder - Feb Batch",
                                          desc: "All pending fees for Feb 2024 batch must be cleared by March 07.",
                                          time: "Admin· 2 hours ago",
                                          icon: "fa-circle-exclamation",
                                          color: "#EF4444",
                                        },
                                        {
                                          title:
                                            "Exam Schedule Published - BCA-II",
                                          desc: "BCA-II internal exams start March 12. Detail cards available at front desk.",
                                          time: "Exam Office· 5 hours ago",
                                          icon: "fa-circle-check",
                                          color: "#10B981",
                                        },
                                        {
                                          title:
                                            "Holiday: Holi - March 14, 2026",
                                          desc: "Institute will remain closed on March 14. Make-up class counts as 1.5.",
                                          time: "Principal Office· Yesterday",
                                          icon: "fa-sun",
                                          color: "#3B82F6",
                                        },
                                      ]
                                    ...
                                    ${
                                      (s.pending_leaves || []).length
                                        ...
                                        <div class="list-item">
                                            ...
                                            .join("")
                                        : '<div style="text-align:center;padding:20px;color:var(--text-light);font-size:12px;">No pending requests</div>'
                                    }
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="panel h-100">
                                ...
                                <div style="margin-top:12px;">
                                    ${(
                                      s.activity_log || [
                                        {
                                          msg: "Rs 2,000 collected from Ramesh Sharma (RCP-000009)",
                                          time: "7:42 PM",
                                          user: "Sunita Devi",
                                        },
                                        {
                                          msg: "Leave request from Bikash KC appvd for March 10",
                                          time: "7:33 PM",
                                          user: "Sunita Devi",
                                        },
                                        {
                                          msg: "Rs 10,000 collected from Priyanka Shah (RCP-000008)",
                                          time: "7:31 PM",
                                          user: "Sunita Devi",
                                        },
                                        {
                                          msg: "New student Priyanka Shah registered (STD-0034) in BCA-I",
                                          time: "7:30 PM",
                                          user: "Sunita Devi",
                                        },
                                      ]
                                    ...
                                    ${
                                      (s.recent_library || []).length
                                        ...
                                        <div class="list-item">
                                            ...
                                            .join("")
                                        : '<div style="text-align:center;padding:20px;color:var(--text-light);font-size:12px;">No recent issues</div>'
                                    }
                                </div>
  ...
  async function loadOutstandingData() {
    ...
    try {
      ...
      const res = await fetch(
        APP_URL + "/api/frontdesk/fees?action=get_outstanding",
        getHeaders(),
      );
      const result = await res.json();
      ...
      const totalOutstanding = outstandingDataCache.reduce((sum, item) => {
        return (
          sum +
          (parseFloat(item.total_due || 0) - parseFloat(item.total_paid || 0))
        );
      }, 0);

      // Count overdue (due date passed)
      const today = new Date().toISOString().split("T")[0];
      const overdueCount = outstandingDataCache.filter(
      ...
      const uniqueCourses = [
        ...new Set(
          outstandingDataCache.map((d) => d.course_id).filter(Boolean),
        ),
      ];
  ...
  window.filterOutstanding = function () {
    const searchTerm =
      document.getElementById("outstandingSearch")?.value?.toLowerCase() || "";
    const courseFilter =
      document.getElementById("outstandingCourseFilter")?.value || "";
    const statusFilter =
      document.getElementById("outstandingStatusFilter")?.value || "";
    const today = new Date().toISOString().split("T")[0];
  ...
  window.renderFeeReceipts = async function () {
    ...
    try {
      const res = await fetch(
        APP_URL + "/api/frontdesk/fees?action=get_recent_payments",
        getHeaders(),
      );
      const result = await res.json();
      ...
      if (result.success && Array.isArray(result.data)) {
        if (result.data.length === 0) {
          container.innerHTML =
            '<div style="padding:40px; text-align:center; color:#94a3b8;">No recent payments found.</div>';
          return;
        }
  ...
  window.renderAnnouncementsList = async function () {
    ...
    try {
      const res = await fetch(
        APP_URL + "/api/frontdesk/announcements?action=list",
        getHeaders(),
      );
      const result = await res.json();
      ...
      if (result.success && result.data) {
        if (result.data.length === 0) {
          container.innerHTML =
            '<div class="col-12"><div class="panel" style="text-align:center; padding:60px; color:var(--text-light);"><i class="fa-solid fa-bullhorn" style="font-size:3rem; opacity:0.1; margin-bottom:15px;"></i><p>No announcements found.</p></div></div>';
          return;
        }
      ...
      document.getElementById("announcementList").innerHTML =
        `<div class="col-12"><div class="alert alert-danger">Error: ${e.message}</div></div>`;
    }
  };
  ...
  window.renderSupportTickets = async function () {
    ...
    try {
      const res = await fetch(
        APP_URL + "/api/frontdesk/support?action=list",
        getHeaders(),
      );
      const result = await res.json();
      ...
      if (result.success && result.data) {
        if (result.data.length === 0) {
          container.innerHTML =
            '<tr><td colspan="7" style="text-align:center; padding:40px; color:var(--text-light);">No support tickets found.</td></tr>';
          return;
        }
      ...
      document.getElementById("ticketList").innerHTML =
        `<tr><td colspan="7" style="text-align:center; color:var(--red);">${e.message}</td></tr>`;
    }
  };
  ...
  window.showNewTicketForm = function () {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const res = await fetch(
            APP_URL + "/api/frontdesk/support?action=create",
            {
              method: "POST",
              ...getHeaders({
  ...
  async function renderVisitorLog() {
    ...
    try {
      const res = await fetch(
        `${APP_URL}/dash/front-desk/visitor-log?partial=true`,
      );
      const html = await res.text();
      ...
      mainContent.innerHTML = `<div class="alert alert-danger">Error loading visitor log</div>`;
    }
  }
  ...
  async function renderAppointmentSchedule() {
    ...
    try {
      const res = await fetch(
        `${APP_URL}/dash/front-desk/appointment-schedule?partial=true`,
      );
      const html = await res.text();
      mainContent.innerHTML = `<div class="pg fu">${html}</div>`;
      const scripts = mainContent.querySelectorAll("script");
      ...
      mainContent.innerHTML = `<div class="alert alert-danger">Error loading appointments</div>`;
    }
  }
  ...
  async function renderCallLog() {
    ...
    try {
      const res = await fetch(
        `${APP_URL}/dash/front-desk/call-log?partial=true`,
      );
      const html = await res.text();
      mainContent.innerHTML = `<div class="pg fu">${html}</div>`;
      const scripts = mainContent.querySelectorAll("script");
      ...
      mainContent.innerHTML = `<div class="alert alert-danger">Error loading call logs</div>`;
    }
  }
  ...
  async function renderComplaints() {
    ...
    try {
      const res = await fetch(
        `${APP_URL}/dash/front-desk/complaints?partial=true`,
      );
      const html = await res.text();
      mainContent.innerHTML = `<div class="pg fu">${html}</div>`;
      ...
      mainContent.innerHTML = `<div class="alert alert-danger">Error loading complaints</div>`;
    }
  }
  ...
  window.performGlobalSearch = async (query) => {
    ...
    try {
      const res = await fetch(
        `${APP_URL}/api/frontdesk/students?action=search&search=${encodeURIComponent(query)}`,
        getHeaders(),
      );
  ...
  async function handleHeaderSearch(query) {
    ...
    try {
      ...
      if (data.data && data.data.length > 0) {
        html +=
          '<div class="search-cat"><i class="fa fa-users"></i> Students Found</div>';
        data.data.forEach((s) => {
          html += `
        ...
        html +=
          '<div style="padding:20px; text-align:center; color:#94a3b8;"><i class="fa fa-search-minus" style="font-size:20px; margin-bottom:10px; display:block;"></i>No matching students found</div>';
      }
      html += "</div>";
  ...
  window.renderFeeDetails = async function () {
    ...
    try {
      const res = await fetch(
        `${APP_URL}/dash/front-desk/fee-details?partial=true`,
      );
      const html = await res.text();
      mainContent.innerHTML = `<div class="pg fu">${html}</div>`;
      const innerScripts = mainContent.querySelectorAll("script");
