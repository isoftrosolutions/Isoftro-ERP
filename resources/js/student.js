...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderDashboard() {
        const attPct = ATTENDANCE_SUMMARY.percentage;
    ...
    // ── PAGE COMPONENTS ──
    function renderClassesToday() {
        mainContent.innerHTML = `
            <div class="pg fu">
                <div class="ph"><div class="pt">Today's Timetable</div><div class="ps">Your class schedule for today · Thursday</div></div>
                <div class="cls-strip">
                    ${TODAY_CLASSES.map((cls,i)=>`
                        <div class="cls-card ${cls.status}">
                            <div class="cls-stripe" style="background:${cls.color}"></div>
            ...
            <div class="pg fu">
                ...
                <div class="g2">
                    ${WEEKLY_SCHEDULE.map((day, i) => `
                        <div class="card">
                            <div class="ct">${day.day}</div>
                            ${day.off ?
                                '<div style="text-align:center;padding:20px;color:var(--text-light);">📅 Holiday</div>' :
                                day.sessions.map((session, j) => `
                                    <div class="att-stat-row">
                                        <span class="att-stat-lbl">${session}</span>
                                        <span class="att-stat-val">${j===0?'<span class="pill pg">Today</span>':''}</span>
                                    </div>
                                `).join('')
                            }
                        </div>
            ...
            <div class="pg fu">
                ...
                <div class="g3 mb">
                    <div class="stat-card" style="border-top:3px solid var(--blue);">
                        ...
                        <div class="stat-val" style="color:var(--blue);">${ATTENDANCE_SUMMARY.percentage}%</div>
                        <div class="stat-lbl">Overall Attendance</div>
                        <div class="stat-sub">${ATTENDANCE_SUMMARY.attended}/${ATTENDANCE_SUMMARY.totalClasses} classes</div>
                    </div>
                    <div class="stat-card" style="border-top:3px solid var(--green);">
                        ...
                        <div class="stat-val" style="color:var(--green);">${ATTENDANCE_SUMMARY.attended}</div>
                    ...
                    <div class="stat-card" style="border-top:3px solid var(--amber);">
                        ...
                        <div class="stat-val" style="color:var(--amber);">${ATTENDANCE_SUMMARY.onLeave}</div>
                ...
                <div class="card">
                    ...
                    <div style="display:flex;align-items:flex-end;gap:8px;height:120px;padding:20px 0;">
                        ${ATTENDANCE_SUMMARY.monthlyTrend.map((val, i) => `
                            <div style="flex:1;display:flex;flex-direction:column;align-items:center;gap:8px;">
                                <div style="width:100%;background:var(--blue);border-radius:4px;height:${val*1.2}px;position:relative;">
                                ...
                                <span style="font-size:10px;color:var(--text-light);">M${i+1}</span>
                            </div>
                        `).join('')}
                    </div>
                </div>
            ...
            <div class="pg fu">
                ...
                <div class="card" style="padding:0;">
                    <table class="tbl">
                        ...
                        <tbody>
                            ${ATTENDANCE_HISTORY.map((rec, i) => `
                                <tr>
                                    <td><span class="nm">${rec.date}</span></td>
                                    <td>${rec.subject}</td>
                                    <td><span class="pill ${rec.status==="present"?"pg":rec.status==="leave"?"py":"pr"}">${rec.status}</span></td>
                                    <td style="color:var(--text-light);font-size:11px;">${rec.markedBy}</td>
                                </tr>
                            `).join('')}
                        </tbody>
                    </table>
            ...
            <div class="pg fu">
                ...
                <div class="card" style="max-width:600px;">
                    ...
                    <div class="form-row">
                        ...
                        <textarea class="form-inp" rows="4" placeholder="Please provide a reason for your leave application..."></textarea>
                    </div>
                    <div class="form-row">
                        <label class="form-lbl">Supporting Document (optional)</label>
                        <div style="border:1.5px dashed var(--card-border);border-radius:10px;padding:18px;text-align:center;cursor:pointer;background:var(--bg);">
                            <div style="font-size:22px;margin-bottom:4px;">📎</div>
                            <div style="font-size:12px;color:var(--text-body);">Upload medical certificate or other document</div>
                        </div>
                    </div>
                ...
                <div class="card" style="margin-top:16px;">
                    ...
                    ${LEAVE_APPLICATIONS.map((lv, i) => `
                        <div class="att-stat-row">
                            <span class="att-stat-lbl">${lv.from} to ${lv.to}</span>
                            <span class="pill ${lv.status==="approved"?"pg":"pr"}">${lv.status}</span>
                        </div>
                    `).join('')}
                </div>
            </div>
            ...
            <div class="pg fu">
                ...
                <div class="g3 mb">
                    <div class="stat-card" style="border-top:3px solid var(--red);">
                        ...
                        <div class="stat-val" style="color:var(--red);">NPR ${FEE_STATUS.outstanding}</div>
                        <div class="stat-lbl">Outstanding Amount</div>
                    </div>
                    <div class="stat-card" style="border-top:3px solid var(--green);">
                        <div class="stat-icon" style="background:var(--green-lt);font-size:18px;">✓</div>
                        <div class="stat-val" style="color:var(--green);">NPR ${FEE_STATUS.paid}</div>
                        <div class="stat-lbl">Paid Amount</div>
                    </div>
                    <div class="stat-card" style="border-top:3px solid var(--blue);">
                        <div class="stat-icon" style="background:var(--blue-lt);font-size:18px;">📅</div>
                        <div class="stat-val" style="color:var(--blue);">${FEE_STATUS.nextDueDate}</div>
                        <div class="stat-lbl">Next Due Date</div>
                    </div>
                </div>
                <div class="card">
                    <div class="ct">Payment History</div>
                    ${FEE_STATUS.history.map((f, i) => `
                        <div class="att-stat-row">
                            <span class="att-stat-lbl">${f.type}</span>
                            <div style="display:flex;gap:8px;align-items:center;">
                                <span class="pill ${f.status==="Paid"?"pg":f.status==="Pending"?"pr":"pb"}">${f.status}</span>
                                <span class="att-stat-val">${f.amount?"NPR "+f.amount:""}</span>
                                ${f.receipt ? `<button class="btn btn-ghost btn-sm" style="margin-left:8px;" onclick="showAlert('Downloading receipt ${f.receipt}')">📄 ${f.receipt}</button>` : ''}
                            </div>
                        </div>
                    `).join('')}
                </div>
    ...
    function renderFeePaymentHistory() {
        mainContent.innerHTML = `
            <div class="pg fu">
                    ...
                    <table class="tbl">
                        ...
                        <tbody>
                            ${PAYMENT_HISTORY.map((p, i) => `
                                <tr>
                                    <td><span class="nm">${p.date}</span></td>
                                    <td>${p.method}</td>
                                    <td style="font-weight:700;">NPR ${p.amount}</td>
                                    <td style="font-family:monospace;font-size:11px;">${p.ref}</td>
                                    <td><span class="pill pg">${p.status}</span></td>
                                </tr>
                            `).join('')}
                        </tbody>
                    </table>
