...
class DashboardCacheService {
    ...
    public function getStats($tenantId) {
        return $this->cache->remember(
            "dashboard:stats:tenant_{$tenantId}",
            $this->ttl,
            function() use ($tenantId) {
                return $this->generateDashboardStats($tenantId);
            }
        );
    }
    ...
    public function invalidate($tenantId) {
        try {
            ...
            \Illuminate\Support\Facades\Log::warning("Redis Cache Error in Dashboard Invalidate: " . $e->getMessage());
        }
    }
    ...
    private function generateDashboardStats($tenantId) {
        ...
        $stats['kpi_dues'] = [
            ...
            'threshold_exceeded' => (float)$pendingData['total_dues'] > 500000
        ];
        ...
        $total = (int)$attToday['total'];
        $stats['attendance_rate'] = $total > 0 ? round(((int)$attToday['present'] / $total) * 100, 1) : 0;
        $stats['attendance'] = [
            'present' => (int)$attToday['present'],
